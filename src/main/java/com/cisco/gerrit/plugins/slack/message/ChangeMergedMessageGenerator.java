...
public class ChangeMergedMessageGenerator extends MessageGenerator
{
    ...
    public String generate()
    {
        ...
        String patchSet = "";
        if (event.change.currentPatchSet != null) {
            patchSet = String.format("patch set #%s of ",
                    event.change.currentPatchSet.number);
        }

        String whatHappened = String.format("%s (%s) merged %s%s",
